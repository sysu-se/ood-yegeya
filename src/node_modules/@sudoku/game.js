...
import { grid, isStateCode, restoreFromStateCode } from './stores/grid';
...
export function startCustom(sencode) {
	...
	if (isStateCode(sencode)) {
		const restored = restoreFromStateCode(sencode);
		if (!restored) return;
	} else {
...
export function enterExplore() {
	return grid.enterExplore();
}

export function submitExplore() {
	return grid.submitExplore();
}

export function abandonExplore() {
	return grid.abandonExplore();
}
